...
#' K-core induced subgraph
#' 
#' The k-core of graph is a maximal subgraph in which each vertex has at least
#' degree k. 
#' 
#' The k-core induced subgraph is the maximal subgraph in which every vertex has at
#' least degree k.
#' 
#' This function calculates the induced subgraph for core 'k'.
#'
#' @param graph The input graph, it can be directed or undirected
#' @param mode The type of the core in directed graphs. Character constant,
#' possible values: \code{in}: in-cores are computed, \code{out}: out-cores are
#' computed, \code{all}: the corresponding undirected graph is considered. This
#' argument is ignored for undirected graphs.
#' @param k The core number for which the subgraph has to be induced.
#' @return Graph object of the induced subgraph
#' @author Chhaya Malik \email{chhaya.mcs.du.2015@gmail.com}
#' @author Mani Bansal \email{mani.mcs.du.2015@gmail.com}
#' @seealso \code{\link{coreness}}
#' @references Vladimir Batagelj, Matjaz Zaversnik: An O(m) Algorithm for Cores
#' Decomposition of Networks, 2002
#' 
...
#' g <- make_ring(10)
#' g <- add_edges(g, c(1,2, 2,3, 1,3))
#' getkcore(g,k=3) 		# small core triangle in a ring
#'
getkcore <- function(graph, mode=c("all", "out", "in"), k) {
  ...
  on.exit( .Call("R_igraph_finalizer", PACKAGE="igraph") )
  res <- .Call("R_dgraph_getkcore", graph, as.numeric(mode), k,
        PACKAGE="dgraph")

  res
}
#' Weighted K-core decomposition of graphs
#' 
...
#'
#' @aliases graph.wtcoreness
#' @param graph The input graph, it can be directed or undirected
#' @param mode The type of the core in directed graphs. Character constant,
#' possible values: \code{in}: in-cores are computed, \code{out}: out-cores are
#' computed, \code{all}: the corresponding undirected graph is considered. This
#' argument is ignored for undirected graphs.
#' @param weights gives the weights of the edges in the graph. The order is determined 
#' by the edge ids. If not specified, the edge attribute weight will be used by default. 
#' @param wtdeg The type of algorithm to be used. Character constant,
...
#'
#' @return Numeric vector of integer numbers giving the weighted coreness of each
#' vertex.
#' @author Chhaya Malik \email{chhaya.mcs.du.2015@@gmail.com}
#' @author Mani Bansal \email{mani.mcs.du.2015@@gmail.com}
#' @seealso \code{\link{wtcoreness}}
#' @references Vladimir Batagelj, Matjaz Zaversnik: An O(m) Algorithm for Cores
#' Decomposition of Networks, 2002
#'
...
#' 
wtcoreness <- function(graph, mode=c("all", "out", "in"), weights=NULL, type=c("first","second"),
			 alpha=NULL, beta=NULL, lambda=NULL, wtdeg=c("once","rep")) {
  ...
  if ((!is.null(alpha) && alpha<=0) || (!is.null(beta) && beta<=0)) {
    stop("`alpha/beta' must be positive")
  }

  type <- igraph.match.arg(type)
  type <- switch(type, "first"=1, "second"=2)

  mode <- igraph.match.arg(mode)
  mode <- switch(mode, "out"=1, "in"=2, "all"=3)
	
  wtdeg <- igraph.match.arg(wtdeg)
  wtdeg <- switch(wtdeg, "once"=1, "rep"=2)

  if(as.numeric(type)==1){
  	if (is.null(alpha)) {
    		warning("`alpha' not set, will be set to 1")
    		alpha <- 1
  	}
  
  	if (is.null(beta)) {
    		warning("`beta' not set, will be set to 1")
    		beta <- 1
  	}
    
	if(!is.null(lambda)){
		warning("`lamba' not needed, will be ignored")
	}
  	if (!is.null(alpha)) { alpha <- as.numeric(alpha) }
  	if (!is.null(beta)) { beta <- as.numeric(beta) }
  }
  
  if(as.numeric(type)==2){
	if(is.null(lambda)){
		warning("`lambda' not set, will be set to 0.5")
    		lambda <- 0.5
        }

	if (!is.null(alpha) || !is.null(beta)) {
    		warning("`alpha/beta' not needed, will be ignored")
  	}
	if (!is.null(lambda)) { lambda <- as.numeric(lambda) }
  }
...
#               PACKAGE="igraph")
 # }else 
  ...
  if (!is.null(weights)){
	if(as.numeric(wtdeg)==1){
    		on.exit( .Call("R_igraph_finalizer", PACKAGE="igraph") )
    		res <- .Call("R_dgraph_wtcoreness1", graph, as.numeric(mode), as.numeric(weights), as.numeric(type), alpha, beta, lambda,
               		PACKAGE="dgraph")
	}     

	else if(as.numeric(wtdeg)==2){
    		on.exit( .Call("R_igraph_finalizer", PACKAGE="igraph") )
    		res <- .Call("R_dgraph_wtcoreness2", graph, as.numeric(mode), as.numeric(weights), as.numeric(type), alpha, beta, lambda,
               		PACKAGE="dgraph")
	}     
  }
 if (igraph_opt("add.vertex.names") && is_named(graph)) {
...
#' Weighted k-core induced subgraph
#' 
#' The k-core of graph is a maximal subgraph in which each vertex has at least
#' degree k. 
#' 
...
#' getkcore(g,k=3) 		# small core triangle in a ring
#'
getwtkcore <- function(graph, mode=c("all", "out", "in"), k, weights=NULL, type=c("first","second"), 
			alpha=NULL, beta=NULL, lambda=NULL, wtdeg=c("once","rep")) {
...
#               PACKAGE="igraph")
 # }else 
  if (is.null(weights) && ! "weight" %in% edge_attr_names(graph) ) {
      stop("Graph unweighted, use getkcore instead")
  }
  ...
  on.exit( .Call("R_igraph_finalizer", PACKAGE="igraph") )
  res <- .Call("R_dgraph_getwtkcore", graph, as.numeric(mode), k, as.numeric(weights), 
	  as.numeric(type), alpha, beta, lambda, as.numeric(wtdeg),
          PACKAGE="dgraph")
...
#' K-truss decomposition of graphs
#' 
#' The k-truss of graph is a maximal subgraph in which each edge participates in least
#' k-2 triangles. The trussness of a vertex is k if it belongs to  k-truss but not
#' to the (k+1)-truss.
#' 
#' The k-truss of graph is a maximal subgraph in which each edge participates in least
#' k-2 triangles.The trusses of a graph form layers: the (k+1)-truss is always a
#' subgraph of the k-truss.
#' 
#' This function calculates the trussness for each edge using algorithm 2 in the reference.
#'
#' @aliases graph.trussness
#' @param graph The input graph ,it is undirected and unweighted
#' @return Numeric vector of integer numbers giving the trussness of each
#' edge.
#' @author Ruchi Goyal \email{ruchigoyal022@gmail.com}
#' @author Sana Zehra \email{sanazehra.mcs.du.2014@gmail.com}
#' @author Sharanjit kaur \email{skaurjit@yahoo.com}
#' @author Vasudha Bhatnagar \email{vasudhabhatnagar@gmail.com}
#' @references Wang, Jia, and James Cheng. "Truss decomposition in massive networks." 
...
trussness <- function(graph) {
  ...
  on.exit( .Call("R_igraph_finalizer", PACKAGE="igraph") )
  res <- .Call("R_dgraph_trussness", graph,PACKAGE="dgraph")
  res
}
